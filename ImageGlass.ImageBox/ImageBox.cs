    ...
    public class ImageBox : Control
    {
        ...
        protected override void OnMouseMove(MouseEventArgs e)
        {
            ...
            if (e.Button == MouseButtons.Right )
            {
                if (isDrag)
                {
                    ...
                    Point endPoint = ((Control)this).PointToScreen(new Point(e.X, e.Y));
        ...
        protected override void OnMouseUp(MouseEventArgs e)
        {
            ...
            if (e.Button == MouseButtons.Right)
            {
                ...
                endPoint = e.Location;
                DrawSelectFrame(g, startPoint2, endPoint); 
            }
            #endregion
        }
        ...
        protected virtual void DrawSelectFrame(Graphics g,Point startPoint,Point endPoint)
        {
            Point LeftTopPoint=new Point();
            LeftTopPoint.X = Math.Min(startPoint.X, endPoint.X);
            LeftTopPoint.Y = Math.Min(startPoint.Y, endPoint.Y);
            Size pSize = new Size();
            pSize.Width = Math.Abs(startPoint.X - endPoint.X);
            pSize.Height = Math.Abs(startPoint.Y - endPoint.Y);
            Rectangle pRectangle = new Rectangle(LeftTopPoint, pSize);
            Pen pen = new Pen(Color.Red,2f);
            g.DrawRectangle(pen, pRectangle);
            DialogResult dr = MessageBox.Show("Are you sure about the screenshot ?", "Notice", MessageBoxButtons.OKCancel);
            if (dr == DialogResult.OK)
            {
                ...
                //RectangleF SelectionRegion_cut = selection;
                Image pImage = null;          
                Rectangle rect;
                rect = FitRectangle(new Rectangle((int)selection.X, (int)selection.Y, (int)selection.Width, (int)selection.Height));
                if (rect.Width > 0 && rect.Height > 0)
                {
                    pImage = new Bitmap(rect.Width, rect.Height);

                    using (Graphics g2 = Graphics.FromImage(pImage))
                    {
                        g2.DrawImage(Image, new Rectangle(Point.Empty, rect.Size), rect, GraphicsUnit.Pixel);
                    }
                }
                this.Image = pImage;
                Graphics pGraphics = this.CreateGraphics();
                //DrawImage(pGraphics);
                //RectangleF pRectangleF = new RectangleF();
                selection.Inflate(selection.Width*1.2f, selection.Height*1.2f);
                ZoomToRegion(selection);
            }
